...
function createWaterAppearance() {
  ...
  appearance.material = Material.fromType('Water', {
    baseWaterColor: new Color(0.06, 0.30, 0.55, 0.30),
    blendColor: new Color(0.06, 0.30, 0.55, 0.10),
...
export default function Map3D() {
  ...
  const renderInterpolatedMeshAtTime = (timeH: number, visualWaveTimeS: number) => {
    ...
    for (const triId of activeTriangleIds) {
      ...
      if (avgDepth <= MIN_ACTIVE_TRIANGLE_DEPTH_M) {
        continue
      }
  ...
  return (
    <div className="map3d-wrapper">
      ...
      {isLoading && (
        <div className="map3d-loading">Loading continuous water mesh...</div>
      )}

      <div className="simulation-panel">
        <div className="simulation-title">Flood simulation</div>
        <div className="simulation-subtitle">
          {dataReady ? '✅ Continuous mesh loaded' : '⏳ Loading data...'}
        </div>
        ...
        <button
          ...
          disabled={!meshFrames}
        >
          {!meshFrames ? 'Loading...' : isAnimating ? 'Stop simulation' : 'Play simulation'}
        </button>
        ...
        {currentPair && (
          <div className="sim-info" style={{ borderColor: activeColor }}>
            <span style={{ color: activeColor, fontWeight: 600 }}>
              {interpolatedFloodedCells} flooded cells
            </span>
      ...
      {buildingInfo && (
        <div className="building-panel">
          ...
          <div className="building-rows">
            ...
            <div className="building-row">
              <span className="building-label">Height</span>
              <span className="building-value">{buildingInfo.height}</span>
            </div>
            <div className="building-row">
              <span className="building-label">Address</span>
