...
import './Map2D.css'
import WW3Legend from './WW3Legend'
...
// ─── NOUVELLES INTERFACES POUR L'INTÉGRATION APP ────────────────────────────

/** Méthodes exposées à App.tsx via onRef */
export interface Map2DRef {
  toggleBdtopo:    () => void
  loadScenario:    (id: string) => void
  clearScenario:   () => void
  toggleWW3:       () => void
  changeWW3Var:    (k: WW3VarKey) => void
  toggleWW3Play:   () => void
  stepWW3:         (delta: number) => void
  toggleWW3Arrows: () => void
}

/** Callbacks remontant l'état vers App.tsx / RightPanel */
interface Map2DCallbacks {
  onScenariosLoaded?: (scenarios: Scenario[]) => void
  onScenarioChange?:  (id: string | null) => void
  onImpactChange?:    (impact: Impact | null) => void
  onBdtopoChange?:    (active: boolean, count: number) => void
...
// ─── COMPOSANT ───────────────────────────────────────────────────────────────
export default function Map2D({ layers, callbacks, onRef }: Props) {
  ...
  // ── Sync état WW3 → RightPanel via callback ────────────────
  useEffect(() => {
    callbacks?.onWW3StateChange?.({
      active:    ww3Active,
      loading:   ww3Loading,
      frame:     ww3Frame,
      var:       ww3Var,
      playing:   ww3Playing,
      arrows:    ww3Arrows,
      frameData: ww3Frames?.frames[ww3Frame] ?? null,
      nFrames:   ww3Frames?.n_frames ?? 0,
    })
  }, [ww3Active, ww3Loading, ww3Frame, ww3Var, ww3Playing, ww3Arrows, ww3Frames])

  // ── Sync bdtopo → App ──────────────────────────────────────
  useEffect(() => {
    callbacks?.onBdtopoChange?.(bdtopoActive, bdtopoCount)
  }, [bdtopoActive, bdtopoCount])

  // ── Sync impact → App ──────────────────────────────────────
  useEffect(() => {
    callbacks?.onImpactChange?.(impact)
  }, [impact])

  // ── Sync scénario actif → App ──────────────────────────────
  useEffect(() => {
    callbacks?.onScenarioChange?.(activeScenario)
  }, [activeScenario])

  // ── Chargement scénarios ───────────────────────────────────
  useEffect(() => {
    fetchJson<Scenario[]>('data/scenarios/index.json')
      .then(data => {
        setScenarios(data)
        callbacks?.onScenariosLoaded?.(data)
      })
      .catch(console.error)
  }, [])
        ...
        new maplibregl.Popup({ maxWidth: '260px' })
          ...
          .setHTML(`
            <div style="font-family:sans-serif;padding:6px 2px">
              <div style="font-weight:700;font-size:13px;margin-bottom:6px;color:#1a1a2e">${props.nature ?? 'Bâtiment'}</div>
  ...
  // ── WW3 toggle arrows ─────────────────────────────────────
  const toggleWW3Arrows = useCallback(() => {
    setWW3Arrows(prev => {
      ww3ArrowRef.current = !prev
      if (!prev) drawWW3Arrows()
      else {
        const c = dirCanvas.current
  ...
  const clearScenario = useCallback(() => {
    ...
    setActiveScenario(null); setImpact(null)
    m.setLayoutProperty('flood-tiles-layer', 'visibility', 'none')
  }, [])

  // ── Exposition des méthodes à App.tsx via onRef ────────────
  useEffect(() => {
    onRef?.({
      toggleBdtopo,
      loadScenario,
    ...
    onRef, toggleBdtopo, loadScenario, clearScenario,
    toggleWW3, changeWW3Var, toggleWW3Play, stepWW3, toggleWW3Arrows,
  ])

  // ─── RENDER ──────────────────────────────────────────────
  // Plus de panels flottants ici — tout est dans RightPanel (App.tsx).
  // Map2D ne rend que la carte, le canvas flèches et le tooltip WW3.
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {/* Tooltip WW3 (au survol d'un point) */}
      <div id="ww3-tooltip" className="ww3-tooltip" style={{ display: 'none' }} />

      {/* Erreur WW3 (flottante discrète en bas de carte) */}
      {ww3Error && (
        <div className="ww3-error-toast">
          ⚠ {ww3Error}
        </div>
      )}
