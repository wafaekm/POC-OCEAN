...
interface WW3Frame {
  ...
  hs_max: number
  hs_mean: number
  cells: Record<string, WW3Cell>
}

interface WW3FramesFile {
  n_frames: number
  frames: WW3Frame[]
}
...
async function fetchJson<T>(path: string): Promise<T> {
  ...
  try {
    ...
    throw new Error(`${path} ne renvoie pas du JSON. Réponse: ${text.slice(0, 120)}`)
  }
}
...
export default function Map2D({ layers }: Props) {
  ...
  useEffect(() => {
    ...
    m.on('load', () => {
      ...
      // Interactions réseaux critiques
      ...
      // Redessiner flèches sur mouvement
  ...
  const startWW3Play = useCallback(() => {
    ...
    ww3PlayRef.current = setInterval(() => {
      const frames = ww3FramesRef.current
      if (!frames) return
      const next = ww3FrameRef.current >= frames.n_frames - 1 ? 0 : ww3FrameRef.current + 1
      applyWW3Frame(next)
    }, 900)
  ...
  const toggleWW3Play = useCallback(() => {
    setWW3Playing(prev => {
      if (prev) { stopWW3Play(); return false }
      startWW3Play(); return true
    })
  ...
  const stepWW3 = useCallback((delta: number) => {
    stopWW3Play()
    setWW3Playing(false)
    applyWW3Frame(ww3FrameRef.current + delta)
  }, [stopWW3Play, applyWW3Frame])
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {/* ── Panel WW3 — juste sous les scénarios ── */}
      <div className="ww3-panel">
        <div className="ww3-header">
          ...
          <button
            className={`ww3-toggle-btn ${ww3Active ? 'active' : ''}`}
            onClick={toggleWW3}
            disabled={ww3Loading}
          >
        ...
        {ww3Active && ww3Loaded && (
          <>
            ...
            <div className="ww3-option-row">
              ...
              <button
                className={`ww3-arrow-btn ${ww3Arrows ? 'active' : ''}`}
                onClick={() => {
                  setWW3Arrows(prev => {
            ...
            <div className="ww3-timeline">
              ...
              <div className="ww3-controls">
                ...
                <button
                  className={`ww3-ctrl-btn ${ww3Playing ? 'active' : ''}`}
                  onClick={toggleWW3Play}
                >
                  {ww3Playing ? '⏸' : '▶'}
                </button>
                <button className="ww3-ctrl-btn" onClick={() => stepWW3(1)}>▶</button>
      ...
      {/* ── NE PAS REMETTRE DE right-stack ICI ──
          LayerControl et Legend sont montés par le parent de Map2D,
          pas à l'intérieur de ce composant. ── */}
    </div>
  )
}
